            ...
            <el-table style="width: 100%" :data="spu.spuSaleAttrList">
                 ...
                 <el-table-column prop="prop" label="属性值名称列表" width="width">
                     <template slot-scope="{row,$index}">
                        ...
                        <el-input class="input-new-tag" v-if="row.inputVisible" v-model="row.inputValue" @blur="handleInputConfirm(row)" size="small"></el-input>
  ...
  methods: {
    ...
    async initSpuData(spu){
      ...
      if (spuImageResult.code == 200) {
        ...
        //由于照片墙显示图片的数据需要数组，数组里面的元素需要有name与url字段 :file-list="spuImageList"
        //需要把服务器返回的数据进行修改
    ...
    async addOrUpdateSpu(){
      ...
      if(result.code=200){
        ...
        //通知父组件回到场景0  
         this.$emit('changeScene',{scene:0,flag:this.category3Id?"修改":"新增"})
         //清除数据
        //  Object.assign(this._data, this.$options.data());
      }
    },
